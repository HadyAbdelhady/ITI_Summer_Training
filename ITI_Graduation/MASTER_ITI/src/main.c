...
u8 h, m, s;		   // To store current time (hours, minutes, seconds)
...
void G_init()
{
	...
	// Set the current time (you may have custom functions for this)
	SetTime(23, 59, 55);
...
void RightID(u8 State)
{
	...
	LCD_String("Right ID @");
	LCD_Send_Data(' ');
	LCD_voidSendNumber(h);
	LCD_Send_Data(' ');
	LCD_voidSendNumber(m);
	LCD_Send_Data(' ');
	LCD_voidSendNumber(s);
...
int main(void)
{
	...
	while (1)
	{
		// Send_TIME();  // Send current time to the slave via SPI
